...
bool ParseRapidJsonEvents::Key(const Ch* str, rapidjson::SizeType size,
bool copy) {
    switch (_state) {
...
bool ParseRapidJsonEvents::String(const Ch* str, rapidjson::SizeType size,
bool copy) {
    switch (_state) {
    case Value:
        ...
        case 'N':
            ...
            if (strcmp(str + 1, "umberLong(") == 0) {
                ...
                if (str[size - 1] != ')')
                    return false;
                long long value;
                if (!convertNumberLong(&value, str + 11, size - 12))
                    return false;
    ...
    case NumberLong:
        long long value;
        if (!convertNumberLong(&value, str + 1, size - 2))
            return false;
        _bob->append(_field, value);
        _state = Unwind;
        return true;
