...
public:
    ...
    InputChunkBatcherHolder(Settings settings, mtools::MongoCluster& mCluster,
            dispatch::ChunkDispatcher* out, mtools::NameSpace ns) :
            _settings(std::move(settings)), _mCluster(mCluster), _out(out), _inputPlan(
                    mtools::MongoCluster::ConfigChunkSort()), _ns(ns) {
        init(_ns);
    }
...
private:
     ...
     */
    void init(const mtools::NameSpace& ns);

    const mtools::MongoCluster& cluster() const {
        return _mCluster;
    }
