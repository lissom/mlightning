...
void metaToStream(std::ostream& out, const FileChunkType blockType, const SequenceId sequenceId,
        const FileChunkFormat formatType, const uint64_t size) {
    out << static_cast<std::underlying_type<FileChunkType>::type>(blockType) << sequenceId
            << static_cast<std::underlying_type<FileChunkFormat>::type>(formatType) << size;
}

void metaFromStream(std::istream& in, FileChunkType* const blockType, SequenceId* const sequenceId,
        FileChunkFormat* const formatType, uint64_t* const size) {
    std::underlying_type<FileChunkType>::type fileChunkHeaderType;
    std::underlying_type<FileChunkFormat>::type fileChunkFormatType;
    in >> fileChunkHeaderType >> *sequenceId >> fileChunkFormatType >> *size;
...
size_t readFromStream(std::istream& in, FileChunkType* blockType, SequenceId* sequenceId,
    ...
    default:
        throw std::range_error("readFromStream: Unknown compression type");
    }
}
