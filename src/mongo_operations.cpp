...
OpQueueBulkInsertUnorderedv24_0::OpQueueBulkInsertUnorderedv24_0(std::string ns, DataQueue* data,
        int flags, const WriteConcern* wc) :
...
OpReturnCode OpQueueBulkInsertUnorderedv24_0::run(Connection* const conn) {
    ...
    if (!_wc || !_wc->requiresConfirmation())
        return OpReturnCode::ok;
    return opCheckError(conn);
}

OpQueueBulkInsertUnorderedv26_0::OpQueueBulkInsertUnorderedv26_0(std::string ns, DataQueue* data,
        int flags, const WriteConcern* wc) :
        _ns(std::move(ns)), _data(std::move(*data)), _flags(flags), _wc(wc) {
}
