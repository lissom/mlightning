...
public:
     ...
     * Sends data to the docbuilder from the simple queue
     */
    virtual void pushSort(BsonPairDeque* q) {
        assert(false);
    }
...
public:
    ...
    struct Settings {
        ...
        int writeConcern;bool directLoad;
    ...
    ChunkDispatcher(Settings settings, mtools::MongoCluster& mCluster, EndPointHolder* eph,
            mtools::NameSpace ns);
    ...
    const mtools::NameSpace& ns() const {
        return _ns;
    }
...
inline void ChunkDispatchInterface::send(mtools::DataQueue* q) {
    switch (_bulkWriteVersion) {
    case 0:
        endPoint()->push(
                mtools::OpQueueBulkInsertUnorderedv24_0::make(owner()->ns(), q, 0,
                        owner()->writeConcern()));
        break;
    case 1:
        endPoint()->push(
                mtools::OpQueueBulkInsertUnorderedv26_0::make(owner()->ns(), q, 0,
                        owner()->writeConcern()));
        break;
    default:
...
public:
    ...
    static ChunkDispatchPointer create(ChunkDispatcher* owner, EndPointHolder* eph, Bson chunkUB) {
        return ChunkDispatchPointer(
                new DiskQueueBoundedFileDispatch(Settings { owner, eph, chunkUB }));
    }
