...
void Loader::Settings::process() {
    ...
    if (!workPath.empty()) {
        ...
        if (boost::filesystem::exists(boost::filesystem::path(workPath))) {
            std::cerr << "workPath already exists, this can be dangerous so mLightning is exiting. "
                    ...
                    "correct. workPath: " << workPath << std::endl;
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (output.endPoints.directLoad)
        output.stopBalancer = true;
    if (input.endPoints.directLoad)
        input.stopBalancer = true;
...
void Loader::Settings::parseShardKey() {
    ...
    if (hashed && count > 1) {
        std::cerr << "MongoDB currently only supports hashing of a single field" << std::endl;
        exit(EXIT_FAILURE);
    }
...
void Loader::dump() {
    if (!boost::filesystem::create_directories(
            _settings.workPath.substr(0, _settings.workPath.size() - 1))) {
        std::cerr << "Unable to create workPath directory: " << _settings.workPath << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    tpFinalize.queueForEach([this] {this->threadFinalizeQueue();});
...
void Loader::load() {
    ...
    setupOutputCluster();
    _chunkDispatch.reset(
            new dispatch::ChunkDispatcher(_settings.dispatchSettings, cluster(), _endPoints.get(),
                    _settings.output.ns()));
    _timerSetup.stop();
    _timerRead.start();
...
void Loader::run() {
     ...
     */
    std::cout << "Threads: " << _settings.threads << " RAM(Mb): " << _ramMax / 1024 / 1024
            << std::endl;
    ...
    if (!_settings.statsFile.empty()) {
        try {
            ...
            if (statsfile.tellp() == 0) {
                statsfile << "\"time(s)\"," << "\"time\"," << "\"bypass\"," << "\"type\","
                        << "\"input time(s)\"," << "\"key\"," << "\"queuing\"," << "\"queue size\","
                        << "\"threads\"," << "\"endpoint conns\"," << "\"wc\"," << "\"note\""
                        << std::endl;
            }
            statsfile << "\"" << timerLoad.seconds() << "\", " << "\"" << loadSeconds / 60 << "m"
                    << loadSeconds % 60 << "s" << "\", " << "\""
                    << _settings.output.endPoints.directLoad << "\", " << "\""
                    << _settings.inputType << "\", " << "\"" << _timerRead.seconds() << "\", "
                    << "\"" << _settings.shardKeyJson << "\", " << "\"" << _settings.loadQueueJson
                    << "\", " << "\"" << _settings.batcherSettings.queueSize << "\", " << "\""
                    << _settings.threads << "\", " << "\"" << _settings.output.endPoints.threadCount
                    << "\", " << "\"" << _settings.dispatchSettings.writeConcern << "\", " << "\""
                    << _settings.statsFileNote << "\"" << std::endl;
