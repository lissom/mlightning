...
public:
    ...
    struct MetaNameSpace {
        ...
        MetaNameSpace(NameSpace ns__, const bool dropped__, mongo::BSONObj key__,
                const bool unique__, const bool virt__ =
                false) :
                ns(std::move(ns__)), dropped(dropped__), key(std::move(key__)), unique(unique__), virt(
                        virt__) {
        }
    };
    struct MetaDatabase {
        DatabaseName name;bool partitioned;
        ShardName primary;bool virt;
        MetaDatabase(DatabaseName name__, const bool partitioned__, ShardName primary__,
                const bool virt__ = false) :
                name(std::move(name__)), partitioned(partitioned__), primary(std::move(primary__)), virt(
                        virt__) {
        }
    };
