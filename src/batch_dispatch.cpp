...
ChunkDispatcher::ChunkDispatcher(Settings settings, mtools::MongoCluster& mCluster,
        EndPointHolder* eph, mtools::NameSpace ns) :
        _settings(std::move(settings)), _tp(_settings.workThreads), _mCluster(mCluster), _eph(eph), _ns(
                std::move(ns)), _loadPlan(_settings.sortIndex) {
...
void ChunkDispatcher::init() {
    ...
    for (auto& iCm : _mCluster.nsChunks(ns())) {
        ...
        try {
            ...
            _loadPlan.back() = ChunkDispatcherFactory::createObject(_settings.loadQueues->back(),
                    this, _eph, std::get<0>(iCm));
        }
    }
...
void DiskQueueBoundedFileDispatch::spill() {
    ...
    const std::string filepath = owner()->workPath() + "chunk" + chunk + fileCountName + ".mls1b";
